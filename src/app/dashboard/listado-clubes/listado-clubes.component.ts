...
import { getCurrentSeasonString, getSelectedSeason, setSelectedSeason } from 'src/app/core/utils/season.utils';
...
export class ListadoClubesComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (getSelectedSeason() != null && getSelectedSeason() != undefined) {
      this.temporadaStoredValue = getSelectedSeason()!.toString();
    }
  ...
  cambioEquiposClub() {
    ...
    if ((this.equiposDelClub - 1) > numEquipos) {
      this.notification.warning(
        `Este club ya tiene ${this.equiposDelClub - 1} equipos creados. Elimina primero los equipos necesarios y repite la acción.`,
        false
      );
      return;
    };
