...
export class PostEditorModalComponent implements OnInit, OnChanges {
  ...
  private loadImageModels(): void {
    this.prospect.getAiModels().subscribe({
      next: data => { this.imageModels = data.image_models || []; this.cdr.markForCheck(); },
    });
  }
  ...
  private _doGenerateThread(): void {
    ...
    this.prospect.generateThreadStandalone({
      ...
      error: (err: any) => {
        ...
        this.cdr.markForCheck();
        setTimeout(() => { this.threadGenError = ''; this.cdr.markForCheck(); }, 6000);
      },
    });
  }
  ...
  generateText(): void {
    ...
    this.prospect.generateSocialPost({
      ...
      error: () => { this.isGenerating = false; this.cdr.markForCheck(); },
    });
  }
  ...
  generateImage(): void {
    ...
    this.prospect.generatePostImage(
      ...
      error: () => { this.isGeneratingImage = false; this.cdr.markForCheck(); },
    });
  }
  ...
  private save(status: string): void {
    ...
    const onSuccess = (result: any) => {
      ...
      this.cdr.markForCheck();
      setTimeout(() => { this.saveFeedback = null; this.cdr.markForCheck(); }, 3000);
    ...
    const onError = () => {
      ...
      this.cdr.markForCheck();
      setTimeout(() => { this.saveFeedback = null; this.cdr.markForCheck(); }, 4000);
    };
