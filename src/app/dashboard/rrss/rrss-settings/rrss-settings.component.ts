...
export class RrssSettingsComponent implements OnInit {
  ...
  // ── Section tabs ──────────────────────────────────────────────────────────
  activeSection: 'redes' | 'conexiones' | 'apikeys' = 'redes';
  ...
  saveToken(key: string): void {
    ...
    this.prospect.updateSocialToken(key, val).subscribe({
      next: () => {
        ...
        setTimeout(() => { this.savedToken[key] = false; }, 2500);
      },
      error: () => { this.savingToken[key] = false; },
    });
  }

  revokeToken(key: string): void {
    this.prospect.updateSocialToken(key, '').subscribe({
      next: () => { this.tokenStatus[key] = false; this.tokenValues[key] = ''; },
    });
  }
  ...
  loadApiKeyStatus(): void {
    this.prospect.getAIKeys().subscribe({
      next: status => { this.apiKeyStatus = status; },
    });
  }
  ...
  saveApiKey(key: string): void {
    ...
    this.prospect.updateSetting(key, val).subscribe({
      next: () => {
        ...
        setTimeout(() => { this.savedApiKey[key] = false; }, 2500);
      },
      error: () => { this.savingApiKey[key] = false; },
    });
  }

  revokeApiKey(key: string): void {
    this.prospect.updateSetting(key, '').subscribe({
      next: () => { this.apiKeyStatus[key] = false; this.apiKeyValues[key] = ''; },
    });
  }
  ...
  save(network: Network): void {
    ...
    this.prospect.updateNetworkConfig(network, {
      ...
      next: () => {
        ...
        setTimeout(() => { this.savedNetwork = null; }, 2500);
      },
