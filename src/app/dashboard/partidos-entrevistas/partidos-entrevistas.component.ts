...
export class PartidosEntrevistasComponent implements OnInit {
  ...
  eliminarImagen(partidoId: number, index: number) {
    if (confirm('¿Seguro que quieres eliminar esta imagen?')) {
      this.galerias[partidoId].splice(index, 1);
    }
  }

  subirImagen(event: any, partidoId: number) {
    const file = event.target.files[0];
    if (file) {
      const url = URL.createObjectURL(file);
      this.galerias[partidoId].push({ urlImg: url, playerId: this.playerId });
    }
  }
